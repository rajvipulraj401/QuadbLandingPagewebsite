...
const PremiumPartners = () => {
  ...
  return (
    <div
      className="container mx-auto relative mt-24"
    ...
    >
      <div className="relative text-white flex flex-col items-center pb-32">
        <h3 className="font-normal text-[22px] leading-[30.8px] text-center">
          Our <span style={textGradient}>Premium Partners</span>
        </h3>
        ...
        <div
          ref={iconsSectionRef}
          className="flex gap-8 xxs:gap-12 sm:gap-16 md:gap-24  lg:gap-32 mt-12 justify-center items-center"
        >
          <img
            src={Edusquarenew}
            alt="Edusquare"
            className={`h-[60px] sm:h-full transition-all duration-[2s] ease-out ${
              isVisible
                ? "opacity-100 translate-y-0 rotate-0"
                : "opacity-0 translate-y-16 rotate-180"
            }`}
          />

          <img
            src={thetopcontroller}
            alt="The Top Controller"
            className={` h-[60px] sm:h-full transition-all duration-[2s] ease-out ${
              isVisible
                ? "opacity-100 translate-y-0 rotate-0"
                : "opacity-0 translate-y-16 rotate-180"
      ...
      <div
        className={`absolute left-[-60%] bottom-0 rounded-full h-full blur-lg opacity-50 ${
          isHovered ? "opacity-70" : ""
        } w-[100%] pointer-events-none`}
        style={{
          backgroundImage:
            "radial-gradient(circle, rgba(240, 246, 0, 0.7), rgba(240, 246, 0, 0.3), rgba(240, 246, 0, 0.1), rgba(84, 85, 66, 0.05), transparent)",
        }}
      ></div>
      <div
        className={`absolute right-[-63%] bottom-0 rounded-full h-full blur-lg opacity-50 ${
          isHovered ? "opacity-70" : ""
        } w-[100%] pointer-events-none`}
        style={{
          backgroundImage:
            "radial-gradient(circle, rgba(240, 246, 0, 0.8), rgba(240, 246, 0, 0.3), rgba(240, 246, 0, 0.1),rgba(84, 85, 66, 0.05), transparent)",
        }}
      ></div>
      <div className="relative text-white flex flex-col items-center pb-12 sm:pb-16 md:pb-20 lg:pb-32 px-4 max-w-5xl mx-auto">
        <h2 className="font-normal text-[22px] leading-[30.8px] text-center">
          Lets <span style={textGradient}>Work Together</span>
        </h2>
        ...
        <h1 className="leading-[30px] sm:leading-[42px] md:leading-[67.2px] text-[23px] sm:text-[32px] md:text-[42px] font-bold mt-6 text-center">
          {/* <h1 className="text-[25px] sm:text-[32px] md:text-[40px] font-bold mt-6 text-center"> */}
